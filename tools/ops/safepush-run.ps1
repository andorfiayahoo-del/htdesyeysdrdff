  ...
  if($status -ne "OK"){
    ...
    if(Test-Path $pub){
      try {
        ...
        if ($LASTEXITCODE -eq 0) { $pubOK = $true }
      } catch {
        Warn ("publisher error: " + $_.Exception.Message)
      }
    }
    if(-not (Test-Path $latestMD)){
      $tail = @()
      try { $tail = Get-Content $tx -Tail 80 } catch { }
      $errTxt = @()
      try { if(Test-Path $errFile){ $errTxt = Get-Content $errFile } } catch { }
    ...
    git -C "$RepoRoot" add -- "ops/live/latest-error.md" "ops/live/latest-pointer.json" "ops/live/transcript_*" "ops/live/error_*" | Out-Null
      ...
      if($changes){
        ...
        Warn "nothing to commit (already captured?)"
      }
    } catch {
      Warn ("commit/push failure: " + $_.Exception.Message)
    }
  }
