param([string]$ProjectRoot = (Get-Location).Path, [string]$ReportPath, [switch]$Soft)
...
if (-not $ReportPath) {
  Write-Host "[validate-match] no report found in ops\live\. Skipping." -ForegroundColor Yellow
  exit 0
}
