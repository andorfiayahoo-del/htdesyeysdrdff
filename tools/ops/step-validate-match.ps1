...
if (-not $ReportPath) { Write-Host "[validate-match] No report, skipping." -ForegroundColor Yellow; exit 0 }
$raw = Get-Content -LiteralPath $ReportPath -Raw
try { $j = $raw | ConvertFrom-Json } catch { $j = $null }
if ($j) {
  $matched=@($j.matched).Count; $mismatched=@($j.mismatched).Count; $missing=@($j.missing).Count
  ...
  Write-Host "[validate-match] Non-JSON report:" -ForegroundColor Cyan
