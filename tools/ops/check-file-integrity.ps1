...
function ReadFileBytes([string]$abs){
  if(Test-Path -LiteralPath $abs){ [System.IO.File]::ReadAllBytes($abs) } else { $null }
}
...
try {
  ...
  if ([string]::IsNullOrWhiteSpace($RunId)) { $RunId = $env:PUSH_RUN_ID }
  ...
  # Working file comparison (optional)
  $workSha = "(skipped)"; $bw = $true
  if (-not $SkipWorking) {
  ...
  # RAW fetch comparison (optional)
  $rawSha = "(skipped)"; $br = $true
  if (-not $SkipRaw) {
  ...
  if (-not $SkipWorking) { Write-Host ("Blob vs Working       : {0}" -f ($(if($bw){"MATCH ✅"} else {"MISMATCH ❌"}))) }
  else                   { Write-Host ("Blob vs Working       : (skipped)") }

  if (-not $SkipRaw)     { Write-Host ("Blob vs RAW           : {0}" -f ($(if($br){"MATCH ✅"} else {"MISMATCH ❌"}))) }
  else                   { Write-Host ("Blob vs RAW           : (skipped)") }

  $anyFail = $false
  if (-not $SkipWorking -and -not $bw) { $anyFail = $true }
  if (-not $SkipRaw     -and -not $br) { $anyFail = $true }
  if ($anyFail) { exit 3 } else { exit 0 }
}
catch {
